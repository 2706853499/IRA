...
class PEER(object):
    def __init__(self, state_dim, action_dim, max_action, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5,
                 policy_freq=2, device: str = 'cpu', hidden_dim=256, **kwargs):
        self.device = device
    ...
    def train(self, replay_buffer, batch_size=256,env=None,sz=None):
        ...
        with torch.no_grad():
            ...
            target_Q1, target_Q2 ,target_feature1, target_feature2= self.critic_target(next_state, next_action)
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + not_done * self.discount * target_Q

        # Get current Q estimates
        current_Q1, current_Q2,current_feature1,current_feature2 = self.critic(state, action)
        # Compute critic loss
        critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)
        peer_loss = torch.einsum('ij,ij->i',[current_feature1,target_feature1]).mean()*self.alpha+torch.einsum('ij,ij->i',[current_feature2,target_feature2]).mean()*self.alpha
